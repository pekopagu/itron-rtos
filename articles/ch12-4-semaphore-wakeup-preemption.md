---
title: "CodexとSDDでμITRON風RTOSを作る 第12章 12.4: セマフォwakeup後の切り替え判定"
emoji: "🛠️"
type: "tech"
...
第12章では、セマフォAPIを題材にtaskの状態遷移を観察できる形で実装しています。

12.1では、`wai_sem()`でセマフォを取得できない場合に、現在RUNNINGのtaskをWAITINGへ落としました。
...
12.2では、`sig_sem()`でWAITING taskをREADYへ戻す経路を追加しました。
...
12.3では、セマフォごとのFIFO wait queueを導入し、`wai_sem()`でenqueue、`sig_sem()`でdequeueする形に整理しました。

今回の12.4では、`sig_sem()`でwakeupしたtaskがcurrent taskより高優先度だった場合に、context switchへ進むための判定を追加します。

ここでのwakeupは、セマフォを待ってWAITINGになっていたtaskをREADYへ戻すことを指します。

今回も、既存RTOSの実装コード参照、コピー、流用は行わず、この学習用RTOSの現在の構造に合わせて実装しています。
...
12.3では、`sig_sem()`がtask table全体を探すのではなく、対象セマフォのwait queueから待ちtaskを取り出すようにしました。
...
ただし、12.3の時点では、READYへ戻した後に「そのtaskへ切り替えるべきか」は扱っていませんでした。

12.4では、この後段にwakeup後のpreemption判定を追加します。

重要なのは、wait queueの解除順は12.3と同じくFIFOのまま維持することです。

今回追加するのは、FIFOで取り出した後のpriority比較です。priority順wait queueへ変更するわけではありません。
...
この点は、12.3で整理したwait queueと待ち理由の管理を壊さないための確認点です。
...
task文脈の`sig_sem()`では、通常のtask実行中にAPIを呼んでいるため、必要になった時点でdispatcherのswitch経路へ進めます。

一方で、timer IRQ handler本体では割り込み処理中なので、直接切り替えずにdispatch pendingとして後段へ渡す、という扱いに分けています。
...
sig_sem(sem_id)
  ...
  woken taskのpriorityがcurrent taskより小さい
    preempt required
...
- `docs/logs/qemu-serial.log`で状態遷移を追えること
...
ふたつ目は、preemption判定のタイミングです。

12.4では、wakeup後にpriority比較を行います。つまり、wait queueからdequeueしただけではなく、taskをREADYへ戻してから比較します。

みっつ目は、同一優先度をswitch対象にしない点です。

同一優先度time sliceやround-robinはまだ未実装なので、同じpriorityのtaskをwakeupしても`wakeup-no-switch`になります。

よっつ目は、wait queueの順序を変えていない点です。

priority順wait queueを実装したわけではなく、12.3のFIFO解除順を維持したまま、dequeue後のtaskに対してpriority比較を追加しています。

最後に、timer IRQ経路と混ぜないことです。

`sig_sem()`はtask文脈APIであり、timer IRQ handlerから呼びません。IRQ中のdispatch pending経路と、task文脈のsemaphore wakeup後switch経路は責務を分けています。
...
未実装:
  ...
  round-robin
  完全な割り込み復帰フレーム切替
  timer IRQ handlerからのsig_sem()呼び出し
```
...
今回もKiro風のspec-driven developmentで進めました。

ここで使っている`$kiro-*`コマンドは、この連載で使っているSDD作業用のコマンドです。要求、設計、タスク、実装、検証の順に作業を段階化するための記録として載せています。
...
```text
.kiro/specs/semaphore-wakeup-preemption/requirements.md
.kiro/specs/semaphore-wakeup-preemption/design.md
.kiro/specs/semaphore-wakeup-preemption/tasks.md
```
...
今回の実装は、この連載で扱っている学習用RTOSリポジトリの次のtag候補に対応します。
...
このプロジェクトでは、学習目的の独自実装として進めています。

既存RTOS実装のソースコード参照、コピー、流用は行っていません。

## 次回以降

次回以降は、今回あえて残した部分を扱っていきます。
...
12.4では、セマフォ待ちtaskをREADYへ戻した後、必要ならcurrent taskからwoken taskへ切り替える入口まで作りました。

ただし、待ちqueue自体はまだFIFOです。

複数task待ちやtimeoutを扱う段階では、wait queueの順序や待ち解除条件をもう少し丁寧に設計する必要があります。
